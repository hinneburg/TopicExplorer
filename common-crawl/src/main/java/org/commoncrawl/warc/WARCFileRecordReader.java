...
public class WARCFileRecordReader extends RecordReader<Text, ArchiveReader> {
    ...
    @Override
    public ArchiveReader getCurrentValue() throws IOException,
        InterruptedException {
        // We only ever have one value to give -- the output of the compressed
        // file
        return ar;
    }
    ...
    public float getProgress() throws IOException, InterruptedException {
        ...
        // As each file only produces one ArchiveReader, this will be one
        // immediately
    ...
    public boolean nextKeyValue() throws IOException, InterruptedException {
        // As each file only produces one ArchiveReader, if it has been read,
        // there are no more
        if (hasBeenRead) {
            return false;
        }
