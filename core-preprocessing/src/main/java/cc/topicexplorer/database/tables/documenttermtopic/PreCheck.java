...
public class PreCheck implements Command {
	...
	public ResultState execute(Context context) {
		...
		if (!properties.get("newTopics").toString().equalsIgnoreCase("true")) {
			try {
				ResultSet topicCountRS = database
						.executeQuery("SELECT COUNT(DISTINCT TOPIC_ID) AS TOPIC_COUNT FROM DOCUMENT_TERM_TOPIC");
				if (topicCountRS.next()) {
					...
					if (!properties.get("malletNumTopics").equals(topicCountRS.getString("TOPIC_COUNT"))) {
						this.userDecision(
								"DOCUMENT_TERM_TOPIC tables topic count (" + topicCountRS.getString("TOPIC_COUNT")
								+ ") doesn't match properties topic count ("
