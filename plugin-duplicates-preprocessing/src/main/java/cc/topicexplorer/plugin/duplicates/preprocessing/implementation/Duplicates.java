...
public class Duplicates {
	...
	public void writeDuplicatesToDB() throws SQLException {
		...
		while (fIt.hasNext()) {
			...
			String query = "INSERT INTO DUPLICATES$DUPLICATES (MD5SUM, DOCUMENT_ID, START_POSITION, END_POSITION, GROUP_ID) "
					+ "VALUES ('"
					+ currentFrame.getMD5Hash()
					+ "', '"
					+ currentFrame.getDocID()
					+ "', '"
					+ currentFrame.getStartPos() + "', '" + currentFrame.getEndPos() + "', '" + groupHash + "')";
	...
	private void processCsvStep1(HashSet<String> duplicateFrameHashes, HashSet<Integer> reprocessDocuments)
			throws IOException {
	...
	private void processCsvStep2(HashSet<String> duplicateFrameHashes, HashSet<Integer> reprocessDocuments)
			throws IOException {
	...
	private void checkFramesForOverlapAndPossiblyUnionThem(Frame prevFrame, Frame currentFrame) {
		...
		if (currentFrame.getStartPos() <= prevFrame.getEndPos()) {
			addToUnionFindStructureAndUnion(unionFind, currentFrame.getMD5Hash(), prevFrame.getMD5Hash());
		}
	}

	private void addToUnionFindStructureAndUnion(UnionFind<String> unionFind, String currentMD5Hash, String prevMD5Hash) {
