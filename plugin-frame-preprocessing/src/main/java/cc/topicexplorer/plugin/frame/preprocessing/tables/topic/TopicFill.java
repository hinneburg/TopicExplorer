...
public class TopicFill extends TableFillCommand {
	...
	private void prepareMetaDataAndFillTable() throws SQLException {
		database.executeUpdateQuery("UPDATE " + this.tableName 
				+ ", (SELECT COUNT(*) AS FRAME_COUNT, COUNT(DISTINCT FRAME) AS UNIQUE_FRAME_COUNT, TOPIC_ID FROM `FRAMES` GROUP BY TOPIC_ID) FRAME"
				+ " SET TOPIC.FRAME$FRAME_COUNT=FRAME.FRAME_COUNT, TOPIC.FRAME$UNIQUE_FRAME_COUNT=FRAME.UNIQUE_FRAME_COUNT"
