...
public class TermTopicFill extends TableFillCommand {
	...
	public void fillTable() throws SQLException {
		...
		for(i = 0; i < rowCount; i++) {
			database.executeUpdateQuery("INSERT INTO " 
					...
					+ topicIds[i] + ","
					+ TERM_TOPIC.TERM_ID.getName() + ","
					+ "SUM(" + TERM_TOPIC.NUMBER_OF_TOKEN_TOPIC.getName() + ")," 
					+ "SUM(" + TERM_TOPIC.PR_TOPIC_GIVEN_TERM.getName() + ")," 
					+ "SUM(" + TERM_TOPIC.NUMBER_OF_TOKEN_TOPIC.getName() + ") / T." 
					...
					+ TERM_TOPIC.getName() + "." 
					+ TERM_TOPIC.TOPIC_ID.getName() + "=TB."
					...
					+ topicIds[i]
					+ " GROUP BY " + TERM_TOPIC.TERM_ID.getName()
	...
	/** MIT-JOOQ-START 
		this.tableName = TERM_TOPIC.getName();
	MIT-JOOQ-ENDE */ 
	/** OHNE_JOOQ-START */ 
		this.tableName = "TERM";
	/** OHNE_JOOQ-ENDE */ 
