...
public class TopicFill extends TableFillCommand {
	...
	private void prepareMetaDataAndFillTable() throws IOException, SQLException {
		...
		String cluster[] = new String[Integer.parseInt(this.properties
				.getProperty("malletNumTopics"))];
		int numTokens[] = new int[Integer.parseInt(this.properties
				.getProperty("malletNumTopics"))];
		...
		rsNumTokens = database.executeQuery("SELECT * FROM " + TOPIC.getName());
		while (rsNumTokens.next()) {
			numTokens[rsNumTokens.getInt(TOPIC.HIERARCHICAL_TOPIC$START
					.getName())] = rsNumTokens.getInt(TOPIC.NUMBER_OF_TOKENS
					.getName());
		}
		MIT-JOOQ-ENDE */ 
		/** OHNE_JOOQ-START */ 
		...
		while (rsNumTokens.next()) {
			numTokens[rsNumTokens.getInt("TOPIC.HIERARCHICAL_TOPIC$START")] = rsNumTokens.getInt("TOPIC.NUMBER_OF_TOKENS");
		}
		/** OHNE_JOOQ-ENDE */
	...
	private void addHierarchicalTopic(int depth, String[] clusterInfos,
			ArrayList<Integer> oldBorders, int[] numTokens) throws SQLException {
		...
		for (int i = 1; i < Integer.parseInt(this.properties
				.getProperty("malletNumTopics")); i++) {
			if (init != Integer.parseInt(clusterInfos[i].split(";")[depth])) {
				if (oldBorders.contains(i)) {
					...
					if (depth == 0) {
						borderStart = 0;
						borderEnd = Integer.parseInt(this.properties
								.getProperty("malletNumTopics")) - 1;
						oldDepth = -1;
					} else {
					/** MIT-JOOQ-START 
					...
					/** MIT-JOOQ-START 
					rsID = database.executeQuery("SELECT MAX("
							+ TOPIC.TOPIC_ID.getName() + ") AS id FROM "
							+ TOPIC.getName());
					MIT-JOOQ-ENDE */ 
					/** OHNE_JOOQ-START */ 
					rsID = database.executeQuery("SELECT MAX("
							+ "TOPIC.TOPIC_ID" + ") AS id FROM "
							+ "TOPIC");
					/** OHNE_JOOQ-ENDE */
					if (rsID.next()) {
					...
					if (depth + 1 < clusterInfos[0].split(";").length) {
						addHierarchicalTopic(depth + 1, clusterInfos,
								oldBorders, numTokens);
					}
				}
			}
		}
	...
	private void newTopic(int id, int start, int end, int depth, int numToken)
			throws SQLException {
		/** MIT-JOOQ-START 
		if (start < end) {
			database.executeUpdateQuery("INSERT INTO " + TOPIC.getName() + "("
					...
					+ TOPIC.HIERARCHICAL_TOPIC$START.getName() + ","
					+ TOPIC.HIERARCHICAL_TOPIC$END.getName() + ","
					+ TOPIC.HIERARCHICAL_TOPIC$DEPTH.getName() + ") VALUES ("
					+ id + "," + numToken + "," + start + ","
					+ end + "," + depth + ")");
		} else if (start == end) {
			database.executeUpdateQuery("UPDATE " + TOPIC.getName() + " SET "
					+ TOPIC.HIERARCHICAL_TOPIC$DEPTH.getName() + "=" + depth 
					+ " WHERE " + TOPIC.HIERARCHICAL_TOPIC$START.getName() + "=" + start
					+ " AND " + TOPIC.HIERARCHICAL_TOPIC$END.getName() + "=" + start);
		}
		MIT-JOOQ-ENDE */ 
		/** OHNE_JOOQ-START */ 
		if (start < end) {
