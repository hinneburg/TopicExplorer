...
public class WikiTextToCSVForeward {
	...
	HashMap<Integer, Integer> bracketPositionsHashMap;
	NavigableSet<Integer> bracketsNavigableSet;
			...
			if (tmpLine.trim().length() > 0) {
				...
				if (pos > -1) {
					...
					try {
						...
						pos = checkIfFoundedPositionIsWithinABracketNaviagbleSet(pos + savedPosition, tmpLine);

					} catch (Exception e) {
						// TODO Auto-generated catch block
						System.err.println("fehler in checkIfPositionOfWordIsWithinBoxBrackets");
						// e.printStackTrace();
					}
					...
					bwlogger.append("catched error, " + wikiTitle + ": " + old_id + " *" + tmpLine + "*:"
							+ (new Integer(pos + savedPosition)) + ": last_savedposition= " + savedPosition + " id:"
							+ old_id + "\n");
				}
			} else {
	...
	private void getBracketsPositionAndPutIntoNavigableSet() {
		...
		putBracketPositionsIntoHashMap();
	...
	private void putBracketPositionsIntoHashMap() {
		...
		// erste und letzte Klammern werden erstmal weggelassen, dh. nur
