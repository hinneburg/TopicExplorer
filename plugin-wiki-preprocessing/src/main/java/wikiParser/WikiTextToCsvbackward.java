...
//		this.failureId = w.getFailureId();
	...
	{
		// searching must be started from the back of the text in cause of the same handling from the others
		
		// assumption: every word can be found in the text (the two parsing texts, 
		// readable text and parsed original text are not equal and could differ in some parts like interpunction and prefix prostfix)
		...
		try
		{
			...
			{
				token = itToken.next();
				pos = text.substring(0, pos).lastIndexOf(token);  // every time the searchstring becomes shorter
				...
				if (pos == -1 && itToken.hasNext())
				{
					try
					{
						bwlogger.append("tokenize readable text: textposition not found, "+ this.wikiTitle + " " + token );
					}
					catch (IOException e)
					{
...
//					macht mit zuviele Fehler wegen Formeln die Brüche haben und 2 Klammern öffnen {{ 
...
//					
					// takes the new position and the line, iterates and returns the new position  
					savedPosition = splitForOutputAndReturnNewPosition(tmpLine, pos);
				}
				else
				{
					//error output to logger
					bwlogger.append("catched error, "+ wikiTitle + ": "+ old_id + " *"+tmpLine + "*:"  + pos + ": last_savedposition= " + savedPosition + "\n");
				}
			}
			else
			{
				savedPosition = savedPosition - tmpLine.length() - 1;	// new line also counts as one character in utf-8 	
			}
		}
		list.clear();
	}
...
//		rekursiv
	 ...
	 * that the csv file for mallet is completely in backward direction
	 * 
	 */
