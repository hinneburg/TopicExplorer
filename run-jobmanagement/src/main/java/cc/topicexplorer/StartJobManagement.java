...
public class StartJobManagement {
	...
	public static void main(String[] args) throws Exception {
		...
		nextJobRS = getNextJobStmt.executeQuery("SELECT COUNT(*) AS RUNNING_JOBS_COUNT FROM TOPIC_EXPLORER WHERE RUNNING IS NOT NULL AND FINISHED IS NULL");
		if(nextJobRS.next()) {
		...
		nextJobRS = getNextJobStmt.executeQuery("SELECT * FROM TOPIC_EXPLORER WHERE RUNNING IS NULL ORDER BY PENDING LIMIT 1");
	...
	private static void generateCsv() {
		...
		try {
			...
			csvWriter.write("\"DOCUMENT_ID\";\"POSITION_OF_TOKEN_IN_DOCUMENT\";\"TERM\";\"TOKEN\";\"WORDTYPE$WORDTYPE\"\n");
			
			JSONParser jsonParser = new JSONParser();
			JSONObject wordtypeItem;
			JSONArray stopWords;
			JSONArray rootArray = (JSONArray) jsonParser.parse(new FileReader("resources" + File.separator + "wordlist.json"));
