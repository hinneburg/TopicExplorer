...
function(ko, $) {
	return function (instance) {
		...
		instance.wordtypeScrollCallback = function(el) {
			...
			if(!instance.wordtypesLoading() && !globalData.Topic[el].WORDTYPES[instance.activeWordType()].wordtypeFull() && $('#topic_' + el).children('div').children('.topicElementContent').height() +  $('#topic_' + el).children('div').children('.topicElementContent').scrollTop() >=  $('#topic_' + el).children('div').children('.topicElementContent')[0].scrollHeight - 35) {
